...
v5.4 §14 hands the next run seven tests "in order of cost." This directory holds the ones that have been **executed**, each with a reproducible script, its numbers, and an honest verdict — including where the verdict is *less* favourable than the canon's wording.
...
| 6 | Refine ◇ to carry three falsifier columns (negation / mirror / reversal) | ☐ **queued** | Opened by §4b: a register that lists only negations measures the anti in halves. Presentation change, not architecture. |
| 7 | Open the CONTRIBUTING issue for rows 9–17 | ☐ **maintainer action** | Those are rows **8–16** in [`../registers.md`](../registers.md) — see the numbering note there. |
